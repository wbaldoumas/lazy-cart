...
public sealed class LazyCartesianProductSeptupleArityPropertyTests
{
    ...
    private readonly IList<float>? _set6;
    private readonly IList<decimal>? _set7;
    private readonly ILazyCartesianProduct<int, string, double, char, bool, float, decimal>? _subjectUnderTest;

    public LazyCartesianProductSeptupleArityPropertyTests()
    {
        _set1 = Enumerable.Range(1, 10).ToList();
        _set2 = Enumerable.Range(1, 10).Select(i => i.ToString(CultureInfo.CurrentCulture)).ToList();
        _set3 = Enumerable.Range(1, 10).Select(i => (double)i).ToList();
        _set4 = Enumerable.Range(0, 26).Select(i => (char)('A' + i)).ToList();
        _set5 = new List<bool> { true, false };
        _set6 = Enumerable.Range(1, 10).Select(i => i * 0.1f).ToList();
        _set7 = Enumerable.Range(1, 10).Select(i => (decimal)i).ToList();
    ...
    {
        if (index.Get < _set1!.Count * _set2!.Count * _set3!.Count * _set4!.Count * _set5!.Count * _set6!.Count *
            _set7!.Count)
        {
            ...
            _set6.Should().Contain(entry.Item6);
            _set7.Should().Contain(entry.Item7);
    ...
    public void IndexOf_ShouldReturnIndex_WhenEntryInProduct(
        ...
        NonNegativeInt item6,
        NonNegativeInt item7)
    {
        var entry = (
            ...
            _set6![item6.Get % _set6.Count],
            _set7![item7.Get % _set7.Count]
        );

        var result = _subjectUnderTest!.IndexOf(entry);

        result.Should().BeInRange(
            0,
            _set1!.Count * _set2!.Count * _set3!.Count * _set4!.Count * _set5!.Count * _set6!.Count * _set7!.Count - 1
        );
    }
    ...
    {
        if (sampleSize.Get < _set1!.Count * _set2!.Count * _set3!.Count * _set4!.Count * _set5!.Count * _set6!.Count * _set7!.Count)
        {
