...
[TestFixture]
public class LazyCartesianProductSeptupleArityTests
{
    ...
    private IList<decimal>? _set7;
    private IList<(int, string, double, char, bool, float, decimal)>? _actualCartesianProduct;
    private IDictionary<(int, string, double, char, bool, float, decimal), int>? _cartesianProductIndices;
    private ILazyCartesianProduct<int, string, double, char, bool, float, decimal>? _subjectUnderTest;
    ...
    public void Setup()
    {
        ...
        _set7 = Enumerable.Range(1, 10).Select(i => i * 1.0m).ToList();
        _actualCartesianProduct = new List<(int, string, double, char, bool, float, decimal)>();
        _cartesianProductIndices = new Dictionary<(int, string, double, char, bool, float, decimal), int>();
                ...
                foreach (var item3 in _set3)
                {
                    foreach (var item4 in _set4)
                    {
                        foreach (var item5 in _set5)
                        {
                            ...
                            {
                                foreach (var item7 in _set7)
                                {
                                    _actualCartesianProduct.Add((item1, item2, item3, item4, item5, item6, item7));
                                    _cartesianProductIndices.Add((item1, item2, item3, item4, item5, item6, item7), index++);
                                }
                            }
                        }
                    }
        ...
        _subjectUnderTest = new LazyCartesianProduct<int, string, double, char, bool, float, decimal>(
            _set1,
            _set2,
            _set3,
            _set4,
    ...
    public void Size_ShouldReturnCorrectSize()
    {
        _subjectUnderTest!.Size
            .Should()
            .Be(_set1!.Count * _set2!.Count * _set3!.Count * _set4!.Count * _set5!.Count * _set6!.Count * _set7!.Count);
    }

    [Test]
    public void AtIndex_ShouldReturnCorrectEntry()
    {
        var maxIndex = _set1!.Count *
                       _set2!.Count *
                       _set3!.Count *
                       _set4!.Count *
                       _set5!.Count *
    ...
    {
        var act = () => _subjectUnderTest!.IndexOf((0, "0", 0.0, '-', false, 0.0f, 0.0m));
    ...
    public void GenerateSamples_ShouldReturnDistinctEntries()
    {
        ...
        var distinctSamples = new HashSet<(int, string, double, char, bool, float, decimal)>(samples);
